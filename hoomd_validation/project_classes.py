...
class LJFluid(ValidationTestProject):
    ...
    def job_statepoints(self):
        """list(dict): A list of statepoints for this project."""
        list_sps = []
        num_particles = 10  # TODO this is just to test things run without failing
        params_list = [(0.75, 0.05),
                       (0.75, 0.8),
                       (1.0, 0.2),
                       (1.0, 0.4),
                       (1.5, 0.6),
        ...
        for kT, density in params_list:
            list_sps.append(
                {"kT": kT, "density": density, "num_particles": num_particles}
            )
        return list_sps
    ...
    def job_document_params(self, job):
        ...
        params_list.append(('nvt_md', dict(seed=seed0,
                                           pressure=0.0,
                                           potential_energy=0.0)))
        params_list.append(('npt_md', dict(seed=seed1,
                                           density=0.0,
                                           potential_energy=0.0)))
        params_list.append(('nvt_mc', dict(seed=seed2,
                                           pressure=0.0,
                                           potential_energy=0.0)))
        params_list.append(('npt_mc', dict(seed=seed3,
                                           density=0.0,
                                           potential_energy=0.0)))
